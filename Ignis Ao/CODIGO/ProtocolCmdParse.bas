    ...
    If Left$(Comando, 1) = "/" Then
        ...
        Select Case Comando
            ...
            Case "/PENAS"
                ...
                Case "/DARMD"
                 If notNullArguments Then
                 Call WriteDarMD(ArgumentosRaw)
                 Else
                 'Avisar que falta el parametro
                 Call ShowConsoleMsg("Faltan parámetros. Utilice /DARMD nick.")
                 End If
                 
                
                Case "/DARORO"
                If notNullArguments Then
                 Call WriteDarOro(ArgumentosRaw)
                  Else
                  'Avisar que falta el parametro
                  Call ShowConsoleMsg("Faltan parámetros. Utilice /DARORO nick.")
                   End If
                   
                    Case "/DARPLATA"
                     If notNullArguments Then
                      Call WriteDarPlata(ArgumentosRaw)
                       Else
                       'Avisar que le faltó algo
                       Call ShowConsoleMsg("Faltan parámetros. Utilice /DARPLATA nick.")
                       End If
