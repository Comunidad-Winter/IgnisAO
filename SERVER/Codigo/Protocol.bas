...
On Error Resume Next
    ...
    Select Case packetID
        ...
        Case ClientPacketID.Meditate                '/MEDITAR
            ...
            Case ClientPacketID.DarMD               '/DarMD
             Call HandleDarMD(UserIndex)
             
        
         Case ClientPacketID.DarOro                '/DarOro
         Call HandleDarOro(UserIndex)
         
         Case ClientPacketID.DarPlata                '/DarPlata
          Call HandleDarPlata(UserIndex)
...
#End If
        ...
        ElseIf Not VersionOK(version) Then
            Call WriteErrorMsg(UserIndex, "Vercion obsoleta")
        Else
...
'***************************************************
    ...
    End With
End Sub
Private Sub HandleDarPlata(ByVal UserIndex As Integer)
 If UserList(UserIndex).incomingData.length < 3 Then
   Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
   Exit Sub
   End If
       ...
       UserName = buffer.ReadASCIIString()
        tUser = NameIndex(UserName)
         If .flags.Privilegios And PlayerType.User Then  'ojo, solo para gms
          Call WriteConsoleMsg(UserIndex, "ole ole, comando para gms papucho", FontTypeNames.FONTTYPE_INFOBOLD)
          Exit Sub   'evitamos que siga la rutina
           End If
           Dim COPA As Obj
           COPA.Amount = 1  'Cuantas copas le daremos?
           COPA.ObjIndex = 157   'Numero de la copa según obj.dat
           Call MeterItemEnInventario(tUser, COPA)  'metemos la copa en el userinventario
            'decoración
             Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El administrador " & .name & " le ha entregado " & COPA.Amount & " copa de plata a " & UserList(tUser).name & " por haber salido segundo en el torneo.", FontTypeNames.FONTTYPE_CONSEJO))
              Call WriteConsoleMsg(tUser, "El administrador " & .name & " te ha entregado " & COPA.Amount & " copa de plata.", FontTypeNames.FONTTYPE_CENTINELA)
               Call .incomingData.CopyBuffer(buffer)
                  End With
Errhandler:
                  Dim error As Long
...
End Sub
Private Sub HandleDarOro(ByVal UserIndex As Integer)
If UserList(UserIndex).incomingData.length < 3 Then
 Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
 Exit Sub
 End If
    ...
    If .flags.Privilegios And PlayerType.User Then  'ojo, solo para gms
     Call WriteConsoleMsg(UserIndex, "ole ole, comando para gms papucho", FontTypeNames.FONTTYPE_INFOBOLD)
      Exit Sub   'evitamos que siga la rutina
       ...
       Dim COPA As Obj
       COPA.Amount = 1  'Cuantas copas le daremos?
        COPA.ObjIndex = 710   'Numero de la copa según obj.dat
        Call MeterItemEnInventario(tUser, COPA)  'metemos la copa en el userinventario
        'decoración
        Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El administrador " & .name & " le ha entregado " & COPA.Amount & " copas de oro a " & UserList(tUser).name & " por haber salido primero en el torneo.", FontTypeNames.FONTTYPE_CONSEJO))
        Call WriteConsoleMsg(tUser, "El administrador " & .name & " te ha entregado " & COPA.Amount & " copa de oro.", FontTypeNames.FONTTYPE_CENTINELA)
        Call .incomingData.CopyBuffer(buffer)
        End With
Errhandler:
         Dim error As Long
...
End Sub
Private Sub HandleDarMD(ByVal UserIndex As Integer)
If UserList(UserIndex).incomingData.length < 3 Then
Err.Raise UserList(UserIndex).incomingData.NotEnoughDataErrCode
Exit Sub
End If
 ...
 'Remove packet ID
  ...
  UserName = buffer.ReadASCIIString()
   ...
   If .flags.Privilegios And PlayerType.User Then  'ojo, solo para gms
    ...
    MD.Amount = 1
    MD.ObjIndex = 402
    Call MeterItemEnInventario(tUser, MD)
    'decoración
    Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El administrador " & .name & " le ha entregado " & MD.Amount & " Espada Mata Dragones a " & UserList(tUser).name & " por haber ganado el Quest.", FontTypeNames.FONTTYPE_CONSEJO))
    Call WriteConsoleMsg(tUser, "El administrador " & .name & " te ha entregado " & MD.Amount & " Espada Mata Dragones.", FontTypeNames.FONTTYPE_CENTINELA)
    Call .incomingData.CopyBuffer(buffer)
    End With
Errhandler:
    Dim error As Long
       ...
       Dim buffer As New clsByteQueue
        Call buffer.CopyBuffer(.incomingData)
         ...
         Gema.ObjIndex = 406
          ...
          'decoración
          Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("El administrador " & .name & " le ha entregado " & Gema.Amount & " Gema Lunar a " & UserList(tUser).name & " por haber estado atendiendo consultas para ser Game Master.", FontTypeNames.FONTTYPE_CONSEJO))
...
'***************************************************
    With UserList(UserIndex)
     ...
     Call SendData(SendTarget.ToAdmins, 0, PrepareMessageConsoleMsg(LCase$(.name) & " GM: " & "Un usuario mando /GM por favor usa el comando /SHOW SOS", FontTypeNames.FONTTYPE_GUILDMSG))
  ...
  Call buffer.CopyBuffer(.incomingData)
   'Remove packet ID
    Call buffer.ReadByte
     ...
     Else
     Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("||Atencion!!: Se Busca el usuario " & UserList(tIndex).name & ", el que lo asesine tendra su recompenza.", FontTypeNames.FONTTYPE_GUILD))
     Call WriteConsoleMsg(tIndex, "Tu eres el usuario mas buscado, ten cuidado!!.", FontTypeNames.FONTTYPE_GUILD)
     Elmasbuscadojeje = UserList(tIndex).name
     End If
      'If we got here then packet is complete, copy data back to original queue
       Call .incomingData.CopyBuffer(buffer)
       End With
...
Select Case UserGenero
   Case eGenero.Hombre
        Select Case UserRaza
            Case eRaza.Humano
                CaraValida = CBool(Cara >= 1 And Cara <= 51)
                Exit Function
            Case eRaza.Elfo
                CaraValida = CBool(Cara >= 101 And Cara <= 122)
                Exit Function
            Case eRaza.Drow
                CaraValida = CBool(Cara >= 200 And Cara <= 210)
                Exit Function
            Case eRaza.Enano
                CaraValida = CBool(Cara >= 301 And Cara <= 319)
                Exit Function
            Case eRaza.Gnomo
                CaraValida = CBool(Cara >= 401 And Cara <= 418)
                Exit Function
        End Select
   Case eGenero.Mujer
        Select Case UserRaza
            Case eRaza.Humano
                CaraValida = CBool(Cara >= 70 And Cara <= 89)
                Exit Function
            Case eRaza.Elfo
                CaraValida = CBool(Cara >= 170 And Cara <= 188)
                Exit Function
            Case eRaza.Drow
                CaraValida = CBool(Cara >= 270 And Cara <= 288)
                Exit Function
            Case eRaza.Enano
                CaraValida = CBool(Cara >= 370 And Cara <= 385)
                Exit Function
            Case eRaza.Gnomo
                CaraValida = CBool(Cara >= 470 And Cara <= 486)
                Exit Function
        End Select
End Select
CaraValida = False
