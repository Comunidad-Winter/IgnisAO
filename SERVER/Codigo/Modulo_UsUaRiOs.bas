...
'***************************************************
    ...
    End With
'Eduardo
If UserList(VictimIndex).name = Elmasbuscadojeje Then
Dim Recom As Obj
Recom.Amount = 1
 Recom.ObjIndex = 402 'Aka si quieren reemplazan por el items suyos a ganar :D
 Call MeterItemEnInventario(AttackerIndex, Recom)
 Call SendData(SendTarget.ToAll, 0, PrepareMessageConsoleMsg("Atencion!!: " & UserList(VictimIndex).name & " ha eliminado a " & UserList(VictimIndex).name & ", (el mas buscado).", FontTypeNames.FONTTYPE_GUILD))
  Call WriteConsoleMsg(AttackerIndex, "||Has matado al mas buscado. Tu recompenza fue Una espada mata dragones.", FontTypeNames.FONTTYPE_GUILD)
  Elmasbuscadojeje = 0
   End If
End Sub
...
With UserList(UserIndex)
    If .flags.Muerto = 1 Then
        ...
        End If
            ...
            tiempo = (Distance + 0) * 1 'segundos
        
        .Counters.goHome = tiempo / 1 'Se va a chequear cada 6 segundos.
