...
'Si es trigger 7
If MapData(UserList(UserIndex).Pos.Map, UserList(UserIndex).Pos.X, UserList(UserIndex).Pos.Y).trigger = 7 Then
...
'Si esta vivo pero no tiene toda la vida
           ...
           If UserList(TempCharIndex).flags.AdminInvisible = 0 Or .flags.Privilegios And PlayerType.Dios Then
                With UserList(TempCharIndex)
                    If LenB(.DescRM) = 0 And .showName Then 'No tiene descRM y quiere que se vea su nombre.
                         If UserList(TempCharIndex).Stats.MinHp < (UserList(TempCharIndex).Stats.MaxHp * 0.01) Then
                          Vidaa = " | Muerto)"
                     ...
                     If Len(UserList(TempCharIndex).desc) > 1 Then
                                  Stat = UserList(TempCharIndex).name & " (" & ListaClases(.clase) & " " & ListaRazas(.raza) & " Nivel " & UserList(TempCharIndex).Stats.ELV & Vidaa & Stat & " - " & UserList(TempCharIndex).desc
                            Else
                                 Stat = UserList(TempCharIndex).name & " (" & ListaClases(.clase) & " " & ListaRazas(.raza) & " Nivel " & UserList(TempCharIndex).Stats.ELV & Vidaa & Stat
                            End If
